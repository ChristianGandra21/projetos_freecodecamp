...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv('projeto_2/adult.data.csv')
    ...
    rich_percentage_min_workers = len(num_min_workers[num_min_workers['salary'] == '>50K']) / len(num_min_workers) * 100

    # What country has the highest percentage of people that earn >50K?
    country_counts = df['native-country'].value_counts()
    rich_counts = df[df['salary'] == '>50K']['native-country'].value_counts()
    rich_percentage_by_country = (rich_counts / country_counts) * 100
    ...
    # Round numeric results to one decimal place for consistency with the tests
